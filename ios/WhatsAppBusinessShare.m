...
#import "WhatsAppBusinessShare.h"

@implementation WhatsAppBusinessShare
...
- (void)shareSingle:(NSDictionary *)options
    ...
    if ([options objectForKey:@"message"] && [options objectForKey:@"message"] != [NSNull null]) {
        ...
        if ([[UIApplication sharedApplication] canOpenURL: [NSURL URLWithString:@"whatsapp-business://app"]]) {
            NSLog(@"WhatsApp installed");
        } else {
            // Cannot open whatsapp
            NSString *stringURL = @"http://itunes.apple.com/app/whatsapp-business/id1386412985";
        ...
        if ([options[@"url"] rangeOfString:@".wam"].location != NSNotFound || [options[@"url"] rangeOfString:@".mp4"].location != NSNotFound) {
            ...
            NSString * urlWhats = whatsAppNumber ? [NSString stringWithFormat:@"whatsapp-business://send?phone=%@&text=%@", whatsAppNumber, text] : [NSString stringWithFormat:@"whatsapp-business://send?text=%@", text];
